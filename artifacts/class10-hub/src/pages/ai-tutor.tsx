...
import {
  Bot, Send, Plus, Trash2, ChevronLeft,
  Sparkles, BookOpen, FlaskConical, Calculator, Globe, Languages, Loader2
} from "lucide-react";
...
const QUICK_PROMPTS = [
  { icon: Calculator, label: "Maths", text: "Prove that √2 is irrational." },
  { icon: FlaskConical, label: "Science", text: "Explain the difference between acids and bases with examples." },
  { icon: BookOpen, label: "English", text: "Summarise the chapter 'A Letter to God'." },
  { icon: Globe, label: "SST", text: "What were the main causes of the rise of nationalism in Europe?" },
  { icon: Languages, label: "Hindi", text: "Surdas ke pad ka arth aur visheshataen samjhaiye." },
  { icon: Sparkles, label: "Tips", text: "Give me top 5 tips to score 95+ in CBSE board exams." },
];
    ...
    <div className="container mx-auto px-4 md:px-6 max-w-6xl">
      <div className="py-4 md:py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-center justify-between">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary mb-2 font-medium text-sm">
              <Sparkles size={14} /> Powered by AI
            </div>
            <h1 className="text-3xl md:text-4xl font-display font-extrabold">AI Study Tutor</h1>
            <p className="text-muted-foreground mt-1">Ask any Class 10 CBSE question — get instant, step-by-step answers</p>
          </div>
          <button
        ...
        <div className="flex gap-4 h-[70vh]">
          ...
          <AnimatePresence>
            {(showSidebar || true) && (
              ...
              >
                {showSidebar && (
                  <button onClick={() => setShowSidebar(false)} className="absolute top-4 right-4 p-2 rounded-full bg-secondary">
                    <ChevronLeft size={20} />
                  </button>
                )}
                <button
          ...
          {/* Chat Area */}
          <div className="flex-1 flex flex-col bg-card border border-border rounded-3xl overflow-hidden shadow-xl">

            {/* Messages */}
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
              {messages.length === 0 && !streaming && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="h-full flex flex-col items-center justify-center gap-6 py-8">
                  <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-xl shadow-primary/20">
                    <Bot size={40} className="text-white" />
                  </div>
                  <div className="text-center">
                    <h2 className="text-2xl font-display font-bold mb-2">Your AI Tutor is Ready!</h2>
                    <p className="text-muted-foreground max-w-sm">Ask me anything — Maths problems, Science concepts, Essay help, or exam tips. I'm here 24/7!</p>
                  </div>
              ...
              {streaming && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-3">
                  <div className="shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white shadow-md">
                    <Bot size={16} />
                  </div>
            ...
            {/* Input */}
            <div className="border-t border-border p-3 md:p-4 bg-background/50">
              <div className="flex items-end gap-3 bg-secondary/60 rounded-2xl p-2 border border-border focus-within:border-primary/50 transition-colors">
                <textarea
                  ref={inputRef}
                  value={input}
