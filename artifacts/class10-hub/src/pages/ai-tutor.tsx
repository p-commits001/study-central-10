...
const QUICK_PROMPTS = [
  { icon: Calculator, label: "Maths", text: "Prove that √2 is irrational." },
  {
    icon: FlaskConical,
    label: "Science",
    text: "Explain the difference between acids and bases with examples.",
  },
  {
    icon: BookOpen,
    label: "English",
    text: "Summarise the chapter 'A Letter to God'.",
  },
  {
    icon: Globe,
    label: "SST",
    text: "What were the main causes of the rise of nationalism in Europe?",
  },
  {
    icon: Languages,
    label: "Hindi",
    text: "Surdas ke pad ka arth aur visheshataen samjhaiye.",
  },
  {
    icon: Sparkles,
    label: "Tips",
    text: "Give me top 5 tips to score 95+ in CBSE board exams.",
  },
];
...
function renderMarkdown(text: string): string {
  ...
  for (const line of lines) {
    if (/^#{1,3}\s/.test(line)) {
      ...
      const cls =
        level === 1
          ? "text-xl font-bold mt-4 mb-2 text-primary"
      ...
      if (!inOl) {
        result.push('<ol class="list-decimal ml-5 space-y-1 my-2">');
        inOl = true;
      }
      ...
      if (!inList) {
        result.push('<ul class="list-disc ml-5 space-y-1 my-2">');
        inList = true;
      }
...
export default function AiTutor() {
  ...
  const deleteConversation = async (convId: number, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await fetch(`${BASE}/api/openai/conversations/${convId}`, {
        method: "DELETE",
      });
...
const sendMessage = async (text?: string) => {
  ...
  try {
        ...
        if (line.startsWith("data: ")) {
          try {
            ...
            if (parsed.content) {
              ...
              setStreamingContent(accumulated); // Turant update karega
            }
            if (parsed.done) {
    ...
    toast({
      ...
      description: "AI slow hai ya internet check karein.",
      variant: "destructive",
    });
    setStreaming(false);
  }
          ...
          <div className="flex-1 flex flex-col bg-card border border-border rounded-3xl overflow-hidden shadow-xl">
                ...
                >
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-muted-foreground max-w-sm">
                      Ask me anything — Maths problems, Science concepts, Essay
                      help, or exam tips. I'm here 24/7!
                    </p>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 w-full max-w-2xl">
                    {QUICK_PROMPTS.map((p) => (
                      ...
                      >
                        <p.icon
                          size={16}
                          className="shrink-0 text-primary group-hover:text-white"
                        />
                        <div>
                          <div className="font-semibold text-xs text-muted-foreground group-hover:text-white/80 mb-0.5">
                            {p.label}
                          </div>
            ...
            <div className="border-t border-border p-3 md:p-4 bg-background/50">
              ...
              <p className="text-xs text-muted-foreground text-center mt-2">
                AI can make mistakes. Always verify important answers with your
                textbook.
              </p>
            </div>
          </div>
